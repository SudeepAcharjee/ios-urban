...
// Stream of admin payment settings containing uploaded QR code
final paymentSettingsConfigProvider = StreamProvider.autoDispose<Map<String, dynamic>>((ref) {
...
class _WorkerPaymentScreenState extends ConsumerState<WorkerPaymentScreen> with SingleTickerProviderStateMixin {
  ...
  static const upiColor = Color(0xFF2029C5);
  
  bool? _isUpi; // null for none selected, false for Cash, true for UPI/Online
  ...
  void _handleRazorpaySuccess(PaymentSuccessResponse response) async {
    ...
    try {
      await FirebaseFirestore.instance.collection('bookings').doc(bookingId).update({
        ...
        if (response.orderId != null) 'razorpayOrderId': response.orderId,
        if (response.signature != null) 'razorpaySignature': response.signature,
      ...
      if (mounted) {
        toastification.show(
          ...
          title: const Text('Online Payment Received!'),
          description: Text('Payment ID: ${response.paymentId}'),
          autoCloseDuration: const Duration(seconds: 4),
        );
        _showSuccessDialog('Razorpay Online');
      }
    } catch (e) {
      if (mounted) {
        toastification.show(
          ...
          title: const Text('Error Updating Booking'),
  ...
  void _handleRazorpayFailure(PaymentFailureResponse response) {
    if (mounted) {
      ...
      toastification.show(
        context: context,
        type: ToastificationType.error,
        title: const Text('Payment Cancelled / Failed'),
        description: Text(response.message ?? 'Payment was not completed'),
        autoCloseDuration: const Duration(seconds: 4),
      );
    }
  }
  ...
  void _launchRazorpayCheckout() {
    final String rawTotal = widget.taskData['totalPrice']?.toString() ?? widget.taskData['totalAmount']?.toString() ?? '0';
    final double amount = double.tryParse(rawTotal.replaceAll(RegExp(r'[^0-9.]'), '')) ?? 0.0;
    if (amount <= 0) {
      toastification.show(
        context: context,
        type: ToastificationType.warning,
        title: const Text('Invalid Amount'),
        description: const Text('Total amount is 0 or invalid.'),
      );
      return;
    }
    ...
    _razorpayService.openCheckout(
      amount: amount,
      name: 'Urban Services',
      description: widget.taskData['title'] ?? 'Service Payment',
      contact: widget.taskData['userPhone'] ?? widget.taskData['phone'],
      email: widget.taskData['userEmail'] ?? widget.taskData['email'],
      notes: {
        if (bookingId != null) 'bookingId': bookingId,
        'serviceTitle': widget.taskData['title'] ?? '',
  ...
  Future<void> _processPaymentCollection() async {
    ...
    // Razorpay checkout is commented out in favor of on-site UPI QR Code collection
    /*
    if (_isUpi == true) {
    ...
    */
    ...
    try {
      ...
      final String selectedMethod = _isUpi! ? 'Online (QR Code)' : 'Cash';
  ...
  Widget build(BuildContext context) {
    final paymentSettingsAsync = ref.watch(paymentSettingsConfigProvider);
    final String qrUrl = paymentSettingsAsync.value?['upiQrUrl']?.toString() ?? '';
    final String totalPrice = widget.taskData['totalPrice']?.toString() ?? '0';
    final String paymentStatus = (widget.taskData['paymentStatus'] ?? '').toString().toLowerCase();
    final bool isAlreadyPaid = paymentStatus == 'paid' || widget.taskData['razorpayPaymentId'] != null;
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.check_circle_rounded, color: cashColor, size: 48),
                  ),
                  ...
                  Text(
                    'The customer has already completed the payment of ₹$totalPrice online during booking.',
                    textAlign: TextAlign.center,
                    style: const TextStyle(fontSize: 14, color: Colors.grey, height: 1.4),
                  ),
                  if (widget.taskData['razorpayPaymentId'] != null) ...[
                    const SizedBox(height: 12),
                    Container(
                      ...
                      child: Text(
                        'Ref: ${widget.taskData['razorpayPaymentId']}',
  ...
  Widget _buildUpiBox(String qrUrl) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.qr_code_2_rounded, color: primaryColor, size: 24),
              ),
              const SizedBox(width: 12),
              const Text(
          ...
          // Admin Uploaded QR Code Display Card
          if (qrUrl.isNotEmpty) ...[
            GestureDetector(
              onTap: () => _showEnlargedQrDialog(qrUrl),
              child: Container(
                ...
                child: Column(
                  children: [
                        ...
                        errorBuilder: (context, error, stackTrace) => Container(
                          ...
                          child: const Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 8),
                              Text('Failed to load QR', style: TextStyle(fontSize: 12, color: Colors.grey)),
                            ],
                          ),
                        ),
                      ),
                    ...
                    Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(Icons.zoom_in_rounded, size: 14, color: Colors.grey.shade600),
                        const SizedBox(width: 4),
                        Text(
                          'Tap to zoom QR Code',
                          style: TextStyle(
            ...
            // Placeholder when admin hasn't uploaded QR yet
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(24),
              decoration: BoxDecoration(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'Admin has not uploaded the UPI QR code yet in the dashboard settings.',
                    textAlign: TextAlign.center,
          ...
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                const Icon(Icons.check_circle_outline_rounded, size: 16, color: primaryColor),
                const SizedBox(width: 8),
                Flexible(
                  child: Text(
          ...
          _buildDetailRow('Payment Mode', 'Online UPI (QR Code)', valueColor: primaryColor, alignLeft: true),
        ],
      ),
    );
  }

  void _showEnlargedQrDialog(String qrUrl) {
    showDialog(
      context: context,
      builder: (ctx) => Dialog(
        backgroundColor: Colors.transparent,
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    '₹${widget.taskData['totalPrice'] ?? widget.taskData['totalAmount'] ?? ''}',
                    style: const TextStyle(
                      fontSize: 24,
                      fontWeight: FontWeight.w900,
                      color: primaryColor,
                  ...
                  const Text(
                    'Scan via any UPI app to complete payment',
