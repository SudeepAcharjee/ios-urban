...
class _WorkerHomeScreenState extends ConsumerState<WorkerHomeScreen> {
  ...
  Widget _buildServiceCard(Map<String, dynamic> task) {
    ...
    final String price = '₹${task['totalPrice'] ?? task['price'] ?? '0'}';
    final rawLoc = task['address'];
    final String location = (rawLoc is Map)
        ? (rawLoc['address'] ?? rawLoc['doorstepAddress'] ?? 'Customer Location').toString()
        : (rawLoc?.toString() ?? 'Customer Location');
