...
class _WorkerTaskDetailScreenState extends ConsumerState<WorkerTaskDetailScreen> {
  ...
  void initState() {
    ...
    final paymentStatus = (widget.taskData['paymentStatus'] ?? '').toString().toLowerCase();
    _isPaid = paymentStatus == 'paid' || widget.taskData['razorpayPaymentId'] != null;
    _getCoordinatesFromAddress();
  }

  Future<void> _getCoordinatesFromAddress() async {
    final rawAddr = widget.taskData['address'];
    final String? address = (rawAddr is Map)
        ? (rawAddr['address'] ?? rawAddr['doorstepAddress'])?.toString()
        : rawAddr?.toString();
                          ...
                          onPressed: (pickedImages.isEmpty || _isUpdating) ? null : () async {
                            ...
                            try {
                              ...
                              if (mounted) {
                                ...
                                if (_isPaid) {
                                  // Customer already paid online! Directly mark status as Pending Verification
                                  await FirebaseFirestore.instance.collection('bookings').doc(widget.taskData['id']).update({
                                  ...
                                  if (mounted) {
                                    ...
                                    toastification.show(
                                      ...
                                      description: const Text('Customer has already paid online. Task marked as Pending Verification.'),
                                      autoCloseDuration: const Duration(seconds: 4),
                                    );
                                  }
                                } else {
                                  // Customer has not paid yet. Navigate to the payment screen
  ...
  Widget build(BuildContext context) {
    ...
    final rawAddr = widget.taskData['address'];
    final String address = (rawAddr is Map)
        ? (rawAddr['address'] ?? rawAddr['doorstepAddress'] ?? 'Location not specified').toString()
        : (rawAddr?.toString() ?? 'Location not specified');
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: _isPaid ? const Color(0xFFEFF6FF) : const Color(0xFFFFFBEB),
                                    borderRadius: BorderRadius.circular(12),
                                    border: Border.all(
                                      color: _isPaid ? const Color(0xFF3B82F6).withOpacity(0.3) : const Color(0xFFF59E0B).withOpacity(0.3),
                                    ),
                                  ),
                                  child: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      Icon(
                                        ...
                                        size: 14,
                                        color: _isPaid ? const Color(0xFF2563EB) : const Color(0xFFD97706),
                                      ),
                                      const SizedBox(width: 5),
                                      Text(
                                        _isPaid ? 'PAID ONLINE' : 'PAY AFTER SERVICE',
                                        style: TextStyle(
                                          color: _isPaid ? const Color(0xFF2563EB) : const Color(0xFFD97706),
                                          fontSize: 12,
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              _isPaid 
                                  ? 'Customer already paid online. Awaiting Admin Verification to finalize the booking.'
