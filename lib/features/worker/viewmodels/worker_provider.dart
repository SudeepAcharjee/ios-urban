  ...
  return FirebaseFirestore.instance
      ...
      .map((snapshot) {
        final list = snapshot.docs
            .where((doc) {
              final data = doc.data();
              final wId = data['workerId'] ?? data['technicianId'];
              return wId == user.uid;
            })
            .map((doc) => {
              ...doc.data(),
        ...
        DateTime parseDate(dynamic v1, dynamic v2) {
          for (final val in [v1, v2]) {
        ...
        list.sort((a, b) {
          final aDate = parseDate(a['createdAt'], a['timestamp']);
          final bDate = parseDate(b['createdAt'], b['timestamp']);
