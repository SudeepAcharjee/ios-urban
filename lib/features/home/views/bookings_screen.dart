                ...
                return RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final String title = (booking['title'] ?? booking['serviceName'] ?? 'Service').toString();
                      final String status = (booking['status'] ?? 'PENDING').toString();
                      final num rawPrice = (booking['totalPrice'] is num)
                          ? booking['totalPrice']
                          : (booking['price'] is num)
                              ? booking['price']
                              : num.tryParse(booking['totalPrice']?.toString() ?? '') ??
                                  num.tryParse(booking['price']?.toString() ?? '') ?? 0;
                      final String amount = '₹${rawPrice.toDouble().toStringAsFixed(2)}';
                      final String date = (booking['date'] ?? 'No Date').toString();
                      final String time = (booking['time'] ?? 'No Time').toString();

                      String address = 'Guwahati, Assam';
                      final rawAddress = booking['address'];
                      if (rawAddress is String && rawAddress.isNotEmpty) {
                        address = rawAddress;
                      } else if (rawAddress is Map) {
                        address = (rawAddress['address'] ??
                                rawAddress['doorstepAddress'] ??
                      ...
                      final String imagePath = (booking['imagePath'] ?? booking['serviceImage'] ?? 'assets/images/car_wash_bg.jpg').toString();
