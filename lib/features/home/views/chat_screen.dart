...
class ChatScreen extends ConsumerStatefulWidget {
  ...
  final String? recipientRole; // 'user', 'worker', 'shop'
...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  String get _chatPath {
    ...
    if (bId.isNotEmpty && bId != 'support' && !bId.startsWith('direct_chats/')) {
      if (bId.startsWith('bookings/') ||
          bId.startsWith('messages/') ||
          bId.startsWith('offers/') ||
          bId.startsWith('support_chats/')) {
        return bId;
      }
      return 'bookings/$bId';
    }
  ...
  void _sendMessage({String? text, String? imageUrl}) {
    if ((text != null && text.trim().isNotEmpty) || imageUrl != null) {
      ...
      // Prevent sending phone numbers
      if (finalMsg != null && _containsPhoneNumber(finalMsg)) {
      ...
      String senderName = (userData?['shopName'] ??
          userData?['businessName'] ??
          userData?['name'] ??
          userData?['fullName'] ??
          userData?['ownerName'])?.toString().trim() ?? '';

      if (senderName.isEmpty || senderName.toLowerCase() == 'professional' || senderName.toLowerCase() == 'workshop') {
        if (widget.recipientRole == 'worker' || widget.recipientRole == 'shop') {
          senderName = 'Customer';
        } else if (widget.recipientRole == 'user') {
          senderName = (userData?['shopName'] ?? userData?['businessName'] ?? userData?['name'] ?? 'Shop').toString();
        } else {
          senderName = 'Provider';
        }
      }
  ...
  Widget _buildAppBarTitle(Color primaryColor) {
    if (widget.recipientId != null &&
        widget.recipientId!.isNotEmpty &&
        widget.recipientId != 'support') {
      final String collection = widget.recipientRole == 'shop'
          ? 'shops'
          : (widget.recipientRole == 'worker' ? 'workers' : 'users');

      return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance.collection(collection).doc(widget.recipientId).snapshots(),
        builder: (context, snapshot) {
          String displayName = widget.providerName;
          String? displayImg = widget.imageUrl;
          bool isVerified = false;
          String subtitle = widget.providerRole;

          if (snapshot.hasData && snapshot.data != null && snapshot.data!.exists) {
            final data = snapshot.data!.data();
            if (data != null) {
              if (widget.recipientRole == 'shop') {
                final sName = (data['shopName'] ?? data['businessName'] ?? data['name'])?.toString();
                if (sName != null && sName.trim().isNotEmpty) displayName = sName.trim();
                final sImg = (data['shopImage'] ?? data['bannerImage'] ?? data['profilePic'] ?? data['imageUrl'])?.toString();
                if (sImg != null && sImg.trim().isNotEmpty) displayImg = sImg.trim();
                final st = (data['status'] ?? '').toString().toLowerCase();
                if (st == 'approved' || st == 'active' || data['isVerified'] == true) isVerified = true;
                subtitle = 'Workshop';
              } else if (widget.recipientRole == 'worker') {
                final wName = (data['name'] ?? data['fullName'])?.toString();
                if (wName != null && wName.trim().isNotEmpty) displayName = wName.trim();
                if (data['profilePic'] != null && data['profilePic'].toString().isNotEmpty) {
                  displayImg = data['profilePic'].toString();
                } else if (data['imageUrl'] != null && data['imageUrl'].toString().isNotEmpty) {
                  displayImg = data['imageUrl'].toString();
                }
                final st = (data['status'] ?? '').toString().toLowerCase();
                if (data['isApproved'] == true || st == 'approved' || st == 'active') isVerified = true;
                subtitle = 'Professional';
              } else {
                final uName = (data['name'] ?? data['fullName'])?.toString();
                if (uName != null && uName.trim().isNotEmpty) displayName = uName.trim();
                final uImg = (data['profilePic'] ?? data['photoUrl'] ?? data['imageUrl'])?.toString();
                if (uImg != null && uImg.trim().isNotEmpty) displayImg = uImg.trim();
                subtitle = 'Customer';
              }
            }
          }
    ...
    return _buildHeaderContent(
      displayName: widget.providerName,
      displayImg: widget.imageUrl,
      subtitle: widget.providerRole,
      isVerified: false,
      primaryColor: primaryColor,
    );
  }
          ...
          child: (displayImg == null || displayImg.isEmpty)
              ? Center(
                  child: widget.avatarType == 'text'
                      ? Text(
                          widget.avatarText ?? (displayName.isNotEmpty ? displayName[0].toUpperCase() : ''),
                      ...
                      : Icon(
                          widget.avatarIcon ?? (widget.recipientRole == 'shop' ? Icons.storefront_rounded : Icons.person),
                          color: widget.avatarBgColor != null ? Colors.white : Colors.grey,
                          size: 20,
                        ),
                )
              : null,
          ...
          child: Column(
            ...
            children: [
              ...
              if (!widget.isReadOnly && widget.providerRole != 'Promo') ...[
                ...
                Text(
                  subtitle.isNotEmpty ? subtitle : 'Online',
                  style: TextStyle(
                    color: primaryColor,
                    fontSize: 12,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: _buildAppBarTitle(primaryGreen),
      ),
      body: Column(
        children: [
          Expanded(
