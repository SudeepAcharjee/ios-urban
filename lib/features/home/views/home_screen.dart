...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  String? _lastPromptedReferralCode;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _checkReferralAutoApply();
    });
  }

  Future<void> _checkReferralAutoApply() async {
    ...
    try {
      ...
      if (data['referredBy'] != null || data['referralCode'] == code) {
        return;
      }

      if (_lastPromptedReferralCode == code) return;
      _lastPromptedReferralCode = code;
      ...
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(Icons.card_giftcard_rounded, color: Color(0xFF2029C5), size: 26),
              SizedBox(width: 10),
              Text('Referral Code Found', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            ],
          ),
          content: Column(
            ...
            children: [
              const Text(
                'An invite code was detected in your clipboard:',
              ...
              const SizedBox(height: 12),
              const Text(
                'Both you and your friend will get 10% OFF on any booking!',
                ...
                try {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('🎉 10% Discount Unlocked! Both you and ${res['referrerName']} got 10% OFF.'),
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 30),
                        // 📱 Subscriptions & Service Highlights
                        SizedBox(
                          height: 165,
                          child: PageView(
                            children: const [
                              SubscriptionBannerCard(),
                              ReferAndEarnBannerCard(),
                              ComingSoonCard(subtitle: 'More exciting services\nare on the way!'),
                            ],
                          ),
                        ),
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        final plan = snapshot.data?.docs.isNotEmpty == true ? snapshot.data!.docs.first.data() : null;
        final planName = plan != null ? (plan['name'] ?? 'Car & Bike Pass') : 'Urban Subscriptions';
        final price = plan != null ? 'Starting ₹${(plan['price'] as num?)?.toInt() ?? 799}' : 'Save up to 40%';
        final serviceImg = plan?['serviceImage']?.toString();
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: ClipRRect(
              ...
              child: Stack(
                children: [
                  // 1. Service Image as Background
                  Positioned.fill(
                    child: (serviceImg != null && serviceImg.isNotEmpty)
                        ? Image.network(
                            serviceImg,
                            fit: BoxFit.cover,
                            errorBuilder: (_, __, ___) => Image.asset(
                              'images/services/car_wash_service.png',
                              fit: BoxFit.cover,
                            ),
                          )
                        : Image.asset(
                  ...
                  // 2. Gradient Overlay Scrim for Text Readability
                  Positioned.fill(
                    child: Container(
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          colors: [
                            const Color(0xFF0A1128).withValues(alpha: 0.92), // Deep sapphire dark
                            const Color(0xFF0F172A).withValues(alpha: 0.75), // Mid slate
                            Colors.black.withValues(alpha: 0.30), // Soft on right so photo is visible
                          ],
                  ...
                  // 3. Top right decorative bolt element
                  Positioned(
                    top: 0,
                    right: 0,
                    child: Container(
                  ...
                  // 4. Card Content (No left image, full width)
                  Padding(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: const Color(0xFF2029C5).withValues(alpha: 0.4),
                                blurRadius: 6,
                              ),
                            ],
                          ),
                        ...
                        // Subheading
                        const Text(
                          'View our subscriptions for more features',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 12,
...
class ReferAndEarnBannerCard extends StatelessWidget {
  const ReferAndEarnBannerCard({super.key});
...
class ComingSoonCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        if (subtitle.toLowerCase().contains('subscription')) {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const SubscriptionsScreen()),
          );
