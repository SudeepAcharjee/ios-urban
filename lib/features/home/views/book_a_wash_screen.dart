...
class _BookAWashScreenState extends ConsumerState<BookAWashScreen> {
  ...
  bool get _isShopService =>
      widget.ownerId != null && widget.ownerId!.trim().isNotEmpty;
  ...
  void initState() {
    super.initState();
    if (_isShopService) {
      _selectedAddress = (widget.shopAddress != null &&
  ...
  void _handlePaymentFailure(PaymentFailureResponse response) {
    if (mounted) {
      ...
      final String msg = response.message ?? 'Payment failed or was cancelled';
  ...
  void _initiatePaymentOrBooking() {
    if (_isSaving) return;
  ...
  Future<void> _saveBookingToFirestore({
    ...
    if (_isSaving && paymentId == null && _selectedPaymentMethod != 'Cash') return;
    ...
    try {
      ...
      String? shopOwnerId = widget.ownerId;
      String? shopName;
      if (shopOwnerId == null || shopOwnerId.isEmpty) {
        try {
          ...
          if (serviceDoc.exists && serviceDoc.data() != null) {
            ...
            shopOwnerId = (sData['ownerId'] ?? sData['shopId'])?.toString();
            shopName = (sData['shopName'] ?? sData['businessName'])?.toString();
          }
        } catch (e) {
          debugPrint('Error retrieving service owner: $e');
        }
      }

      if ((shopOwnerId != null && shopOwnerId.isNotEmpty) &&
          (_selectedAddress == 'Select Address' || _selectedAddress.isEmpty)) {
        _selectedAddress = (widget.shopAddress != null &&
                widget.shopAddress!.trim().isNotEmpty)
            ? widget.shopAddress!
            : 'At Workshop / Store';
      ...
      final bookingData = {
        ...
        if (shopOwnerId != null) 'shopId': shopOwnerId,
        if (shopOwnerId != null) 'ownerId': shopOwnerId,
      ...
      // 🔔 Send Booking Request Notification to Workshop / Shop Owner
      if (shopOwnerId != null && shopOwnerId.isNotEmpty) {
        try {
          ...
          await rtdb.ref('notifications/$shopOwnerId').push().set({
            'title': 'New Booking Request! 🚗',
            'message': '${user.displayName ?? "A customer"} requested "${widget.title}" for ₹$_grandTotal.',
          ...
          debugPrint('Error sending notification to shop owner: $e');
        }
      }

      // If a referral reward coupon was used, mark it as used
      if (_appliedCoupon != null) {
        ReferralService.markCouponUsed(_appliedCoupon!, user.uid);
      }
  ...
  Widget _buildPaymentStep() {
    return Column(
      ...
      children: [
        ...
        _buildPaymentSectionHeader('Online Payment (Instant & Secure)'),
        _buildPaymentTile(
          'Pay Online',
          'UPI (GPay, PhonePe, Paytm), Cards, NetBanking & Wallets',
          'Razorpay',
          iconData: Icons.bolt_rounded,
          iconColor: const Color(0xFF2029C5),
          iconSize: 26,
          isComingSoon: true,
        ),
        const SizedBox(height: 20),
  ...
  Widget _buildFooter(bool isConfirmed) {
    ...
    bool isAddressSelected =
        _isShopService || _selectedAddress != 'Select Address';
    ...
    if (_currentStep == 0) {
      ...
      footerButton = ElevatedButton(
        onPressed: _isSaving ? null : _initiatePaymentOrBooking,
        style: ElevatedButton.styleFrom(
          backgroundColor: primaryColor,
          foregroundColor: Colors.white,
          shape: RoundedRectangleBorder(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              isRazorpay
                  ? 'Pay ₹${_grandTotal.toInt()} with Razorpay'
                  : 'Confirm Booking',
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: (tempDateIndex != -1 && tempTime != null)
                        ? () async {
                            try {
                              if (tempDateIndex == 0 && !_isShopService) {
                            ...
                            if (_isShopService) {
                              setState(() {
                                _currentStep = 1; // Move directly to payment for store services
                              });
                            } else {
