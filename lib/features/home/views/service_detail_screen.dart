...
class _ServiceDetailScreenState extends ConsumerState<ServiceDetailScreen> {
              ...
              children: [
                ...
                Transform.translate(
                  ...
                  child: Container(
                    ...
                    child: Padding(
                      padding: EdgeInsets.fromLTRB(25, 30, 25, 160 + MediaQuery.of(context).padding.bottom),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // ⭐ Rating Row
                          ...
                          // 🏪 Workshop / Service Provider Card (Only shown for services added by a shop)
                          _buildShopDetailsCard(primaryColor, textPrimary, textSecondary),
                          ...
                          ref.watch(reviewsProvider(widget.service.name)).when(
                            data: (reviews) => Column(
                              children: [
                                ...
                                _buildSimplifiedReviews(reviews),
                              ],
                            ),
                            loading: () => const Center(child: Padding(
                              padding: EdgeInsets.symmetric(vertical: 20),
                              child: CircularProgressIndicator(color: Color(0xFF2029C5)),
                            )),
                            error: (err, _) => _buildSimplifiedReviews([]),
                          ),
                        ],
                      ),
                    ),
  ...
  void _navigateToBooking() {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => BookAWashScreen(
      ...
      shopAddress: widget.service.address,
    )));
  }
  ...
  Widget _buildSimplifiedReviews(List<ReviewModel> reviews) {
    if (reviews.isEmpty) {
      return Container(
        ...
        child: Column(
          ...
          children: [
            Stack(
              ...
              children: [
                Icon(Icons.chat_bubble_outline_rounded, size: 80, color: const Color(0xFF2029C5).withValues(alpha: 0.1)),
    ...
    return Column(
      children: reviews.take(5).map((review) => _buildReviewItem(review, const Color(0xFF2029C5), const Color(0xFF6B7280))).toList(),
    );
  }
  ...
  Widget _buildShopDetailsCard(Color primaryColor, Color textPrimary, Color textSecondary) {
    if (widget.service.ownerId == null || widget.service.ownerId!.trim().isEmpty) {
      return const SizedBox.shrink();
    }

    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance.collection('shops').doc(widget.service.ownerId).snapshots(),
      builder: (context, snapshot) {
        ...
        final String shopName = (shopData['shopName'] ?? shopData['businessName'] ?? 'Workshop').toString();
        final String? shopImage = (shopData['shopPhoto'] ??
                shopData['profilePic'] ??
                shopData['shopImage'] ??
                shopData['image'] ??
                shopData['imageUrl'])
            ?.toString();
        final String serviceType = (shopData['serviceType'] ?? widget.service.category).toString();
        final String shopAddress = (shopData['address'] ?? shopData['shopAddress'] ?? widget.service.address ?? '').toString();
        final bool isVerified = (shopData['status']?.toString().toLowerCase() == 'approved') ||
            (shopData['isVerified'] == true);
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Container(
                    ...
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(Icons.storefront_rounded, size: 13, color: primaryColor),
                        const SizedBox(width: 4),
                        Text(
              ...
              // Shop Location (Only Photo, Name, Location as requested)
