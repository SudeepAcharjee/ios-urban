...
class _OtpScreenState extends ConsumerState<OtpScreen> {
  ...
  void _handleNavigation() async {
    ...
    try {
      // Check shops collection first
      var shopDoc = await FirebaseFirestore.instance.collection('shops').doc(user.uid).get();
      bool isShop = shopDoc.exists;

      var doc = isShop ? shopDoc : await FirebaseFirestore.instance.collection('workers').doc(user.uid).get();
      bool isWorker = !isShop && doc.exists;
      ...
      if (data == null) {
      ...
      } else if (isShop || data['role']?.toString().toLowerCase() == 'shop') {
        ...
        if (!onboardingCompleted) {
          targetScreen = ShopOnboardingScreen(initialShopData: data);
        } else if (shopStatus == 'active' || shopStatus == 'approved') {
          targetScreen = const ShopMainScreen();
        } else {
          targetScreen = const ShopWaitingApprovalScreen();
        }
      } else if (isWorker || data['role']?.toString().toLowerCase() == 'worker') {
        final bool hasPhoneVerified = !kRequirePhoneVerification || data['phoneVerified'] == true;
        final bool hasProfile = data['name'] != null && data['phone'] != null && hasPhoneVerified;
        if (!hasProfile) {
          targetScreen = const CompleteProfileScreen();
        } else {
        ...
        final bool hasPhoneVerified = !kRequirePhoneVerification || data['phoneVerified'] == true;
        final bool hasNameAndPhone = data['name'] != null && data['name'].toString().isNotEmpty && data['phone'] != null && data['phone'].toString().isNotEmpty;
        final bool hasProfile = (hasNameAndPhone || data['gender'] != null) && hasPhoneVerified;
      ...
      if (mounted) {
        ...
        if (user != null) {
          try {
            final sDoc = await FirebaseFirestore.instance.collection('shops').doc(user.uid).get();
            if (!mounted) return;
            if (sDoc.exists) {
              final sStatus = (sDoc.data()?['status'] ?? 'pending').toString().toLowerCase();
              if (sStatus == 'active' || sStatus == 'approved') {
                Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const ShopMainScreen()), (route) => false);
              } else {
                Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const ShopWaitingApprovalScreen()), (route) => false);
              }
              return;
            }
          } catch (_) {}
