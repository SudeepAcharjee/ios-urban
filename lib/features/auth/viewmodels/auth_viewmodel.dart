...
class AuthViewModel extends AsyncNotifier<void> {
  ...
  Future<void> login(String email, String password, {String? expectedRole}) async {
    ...
    try {
      ...
      try {
        ...
        // 2. Check if user exists in 'shops', 'workers', or 'users' collection
        if (userCredential.user != null) {
          final String uid = userCredential.user!.uid;

          // Try fetching from shops first
          var shopDoc = await _firestore.collection('shops').doc(uid).get();
          bool isShop = shopDoc.exists;

          // Try fetching from workers
          var workerDoc = !isShop ? await _firestore.collection('workers').doc(uid).get() : null;
          bool isWorker = workerDoc?.exists ?? false;

          // Try fetching from users
          var userDoc = (!isShop && !isWorker) ? await _firestore.collection('users').doc(uid).get() : null;
          bool isUser = userDoc?.exists ?? false;

          final doc = isShop ? shopDoc : (isWorker ? workerDoc! : userDoc);
          ...
          // Enforce expected role if passed from UI
          if (expectedRole != null) {
            final normalizedExpected = expectedRole.toLowerCase();
            if (normalizedExpected == 'shop' && !isShop) {
              await _auth.signOut();
              throw 'This account is not registered as a Shop. Please select the ${isWorker ? "Worker" : "User"} tab to log in.';
            } else if (normalizedExpected == 'worker' && !isWorker) {
              await _auth.signOut();
              throw 'This account is not registered as a Worker. Please select the ${isShop ? "Shop" : "User"} tab to log in.';
            } else if (normalizedExpected == 'user' && !isUser) {
              await _auth.signOut();
              throw 'This account is registered as a ${isShop ? "Shop" : "Worker"}. Please select the ${isShop ? "Shop" : "Worker"} tab to log in.';
            }
          }
          ...
          if (token != null) {
            final collectionName = isShop ? 'shops' : (isWorker ? 'workers' : 'users');
        ...
        if (e.code == 'user-not-found' || e.code == 'invalid-credential') {
          // Check all collections by email
  ...
  Future<void> submitShopOnboarding({
    required String uid,
      ...
      await _auth.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          ...
          try {
            ...
            if (linkEx.code == 'provider-already-linked') {
              try {
  ...
  Future<void> _onSocialSignIn(
    ...
    if (userCredential.user != null) {
      ...
      // 1. Check shops collection first - shops are strictly not allowed to use Google
      var shopDoc = await _firestore.collection('shops').doc(user.uid).get();
      if (shopDoc.exists) {
        await _auth.signOut();
        try {
          await GoogleSignIn().signOut();
        } catch (_) {}
        throw 'Google login is not supported for Shop accounts. Please use email and password under the Shop tab.';
      }
  ...
  return authState.when(
    data: (user) {
      ...
      return Stream.multi((controller) {
        ...
        void subscribeTo(String collection) {
